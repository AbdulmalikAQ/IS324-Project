...
class Login:
    def __init__(self, main, args = {}):
        self.main = main
        self.main.window.title("KSU Golf Carts - Login")
        ...
        ttk.Button(self.frame, text="Signup", command=self.signup).place(x=290, y=325)

    def login(self):
        if not self.id.get():
            return messagebox.showerror(title="Missing Data", message="You must write your ID.")
        ...
        if len(self.id.get()) != 10 and len(self.id.get()) != 6:
            return messagebox.showerror(title="Wrong Data", message="ID must be 10 numbers for Students and 6 numbers for Faculty and Employees.")
